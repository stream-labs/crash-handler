...
HttpHelper::Result HttpHelper_WIN::Request(Method method, std::string_view url, const Headers& requestHeaders,
        std::string_view body, Headers* responseHeaders, std::string* response)
{
    VARIANT varFalse;
    ...
    _bstr_t bstrMethod;
    switch (method) {
        case Method::Get: bstrMethod = L"GET"; break;
        case Method::Post: bstrMethod = L"POST"; break;
    ...
    hr = CoCreateInstance(clsid, NULL, CLSCTX_INPROC_SERVER,
        IID_IWinHttpRequest, &winHttpRequest);
    if (FAILED(hr)) {
        return Result::CouldNotInit;
    }
...
HttpHelper::Result HttpHelper_WIN::GetRequest(std::string_view url, const Headers& requestHeaders,
    Headers* responseHeaders, std::string* response)
{
    return Request(Method::Get, url, requestHeaders, "", responseHeaders, response);
}

HttpHelper::Result HttpHelper_WIN::PostRequest(std::string_view url, const Headers& requestHeaders,
    std::string_view body, Headers* responseHeaders, std::string* response)
{   
